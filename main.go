...
func init() { //Функция init запускается раньше функции main
...
func printIntro(update *tgbotapi.Update) { //отправляем одно сообщение пользователю с некоторой задержкой
...
func showMenu() {
	msg := tgbotapi.NewMessage(gChatID, "Choose one:")
	msg.ReplyMarkup = tgbotapi.NewInlineKeyboardMarkup(
...
func findActivity(actvities Activities, choiceCode string) (activity *Activity, found bool) {
	for _, activity := range actvities {
...
func updateProcessing(update *tgbotapi.Update) {
	...
	if !found {
		if user, found = storeUserFromUpdate(update); !found {
			gBot.Send(tgbotapi.NewMessage(gChatID, "Unable to identify the user"))
			return
		}
	}
...
func main() {
	...
	updateConfig := tgbotapi.NewUpdate(0)        //For getting all updates from User
	updateConfig.Timeout = UPDATE_CONFIG_TIMEOUT //Wait 60 sec from User, if no action - stop the program
	//Using UPDATE_CONFIG_TIMEOUT helps change timeout sec in constants

	for update := range gBot.GetUpdatesChan(updateConfig) {

		//Как раз те самые пользовательские кнопочки, которые таким образом возвращаются
